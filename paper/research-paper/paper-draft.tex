...
% ── AUTHORS ─────────────────────────────────────────────────────────────────
\author{
\IEEEauthorblockN{1\textsuperscript{st} Dev Aggarwal}
...
\and
\IEEEauthorblockN{2\textsuperscript{nd} Devesh Saini}
\IEEEauthorblockA{\textit{Information Technology Department} \\
\textit{Netaji Subhas University of Technology}\\
New Delhi, India \\
...
of 12--16~ms, approximately 25{,}000$\times$ faster than the
theoretical BGP re-convergence baseline, while incurring approximately
$2\times$
...
\begin{enumerate}
  ...
  \item A protocol-layer model of SR-MPLS vs.\ SRv6 recomputation
        latency scaling with domain count, informed by published
        PCEP benchmarks.
\end{enumerate}
...
Fig.~\ref{fig:r1bar} illustrates the H-PCE architecture with ADSO flow.
...
For instance, a domain containing three parallel links of 100\,Mbps
each and a domain containing a single 300\,Mbps link produce identical
max-bandwidth reports; the metrics are many-to-one over topology space.
...
The Child PCE is implemented as a Python module that:
\begin{itemize}
...
The Parent PCE is a multi-threaded TCP server that:
\begin{itemize}
...
\hline
S1 --- Link Failure        & $17.1 \pm 5.0$ & 403{,}000 & 6.6  \\
S2 --- ASBR Degradation    & $12.7 \pm 3.4$ & 403{,}000 & 6.6  \\
S3 --- ASBR Down           & $16.7 \pm 4.8$ & 403{,}000 & 6.6  \\
S4 --- Recovery            & $10.5 \pm 3.0$ & 403{,}000 & 6.6  \\
\hline
\multicolumn{4}{l}{\scriptsize $^{*}$\,Measured (mean $\pm$ std, $N{=}5$ independent trials):}\\
\multicolumn{4}{l}{\scriptsize \phantom{$^{*}$}\,ADSO receipt $\to$ SR push; excludes poll interval.}\\
...
ADSO achieves mean re-convergence of 10.5--17.1\,ms (with standard
deviations of 3.0--5.0\,ms) across all four scenarios over five
independent trials. The no-notification baseline requires
$\sim$403{,}000\,ms ($\sim$6.7~minutes), yielding an ADSO speedup of
approximately 25{,}000$\times$. The full-topology oracle achieves
$\sim$6.6\,ms, limited only by recomputation and PCEP push. The ADSO
overhead relative to the oracle is approximately $2\times$,
attributable to the TCP messaging pipeline (binary encoding, transport,
decoding).
...
threshold $\tau_{bw}$ (10--30\% in 5\% steps). These values are
derived from a simulation sweep over threshold parameters and represent
expected re-convergence times under varied configurations; they differ
from the measured results in Table~\ref{tab:r1}, which report the
actual testbed performance at the chosen operating point.
Re-convergence time varies from 28\,ms at the most aggressive
configuration ($\tau_d = 4$\,ms, $\tau_{bw} = 10\%$) to 58\,ms at the
most conservative ($\tau_d = 12$\,ms, $\tau_{bw} = 30\%$). The star
marker ($\bigstar$) at ($\tau_d = 8$\,ms, $\tau_{bw} = 20\%$)
identifies the chosen configuration, which balances responsiveness
against false-trigger avoidance.
...
Child PCE (Python) & Custom implementation & \checkmark \\
Parent PCE (Python) & Custom implementation & \checkmark \\
...
Parent PCE, and are conjectured to be insufficient to reveal any
internal topology information of the reporting domain---a property that
follows from each metric being a scalar aggregate over all internal
paths (Section~\ref{sec:proposed}). These metrics are carried
...
\begin{enumerate}
  ...
  \item \textbf{Extended statistical study:} Extending the repeated-trial
        analysis to additional failure patterns, larger domain counts,
        and varied traffic loads, with confidence interval analysis to
        further characterise the distribution of re-convergence times.
\end{enumerate}
...
\bibitem{b_bgpconv2}
P.~Francois, C.~Filsfils, J.~Evans, and O.~Bonaventure,
``Achieving sub-second IGP convergence in large IP networks,''
\textit{ACM SIGCOMM Comput.\ Commun.\ Rev.}, vol.~35, no.~3,
pp.~35--44, Jul.\ 2005.
